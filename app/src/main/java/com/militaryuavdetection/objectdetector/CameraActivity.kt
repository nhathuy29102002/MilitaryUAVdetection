...
class CameraActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (captureMode != "REALTIME") {
            binding.cameraBboxOverlay.visibility = android.view.View.GONE
        }
    }
    ...
    private fun setupListeners() {
        binding.btnCapture.setOnClickListener {
            when (captureMode) {
                ...
                "REALTIME" -> { /* Realtime không cần nút capture */ }
            }
        }

        binding.btnCloseCamera.setOnClickListener { finish() }

        // TODO: Thêm nút chuyển camera (front/back) vào activity_camera.xml
        // binding.btnSwitchCamera.setOnClickListener { switchCamera() }
    }

    private fun observeViewModel() {
        if (captureMode == "REALTIME") {
            viewModel.realtimeResults.observe(this) { results ->
                binding.cameraBboxOverlay.setResults(results, viewModel.markType.value ?: 3)
            }
        }
    }
            ...
            when (captureMode) {
                "REALTIME" -> {
                    ...
                    imageAnalyzer = ImageAnalysis.Builder()
                        .setTargetAspectRatio(AspectRatio.RATIO_16_9) // Hoặc 1:1
    ...
    // Lớp nội bộ để phân tích realtime
    private inner class RealtimeAnalyzer : ImageAnalysis.Analyzer {
        ...
        override fun analyze(imageProxy: ImageProxy) {
            ...
            // Giới hạn tốc độ phân tích (ví dụ: 10 FPS)
            if (currentTimestamp - lastAnalyzedTimestamp < 100) { // 100ms = 10 FPS
                imageProxy.close()
                return
            }
    ...
    private fun takePhoto() {
        ...
        val photoFile = File(
            cacheDir, // Lưu tạm, sau đó gửi về ViewModel để xử lý/lưu
        ...
        imageCapture.takePicture(
            ...
            object : ImageCapture.OnImageSavedCallback {
                ...
                override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                    ...
                    // TODO: Gửi photoFile.path về MainActivity/ViewModel để xử lý
                    finish()
                }
            }
        )
    ...
    private fun toggleVideoRecording() {
        ...
        if (recording != null) {
            ...
            recording?.stop()
            recording = null
            binding.btnCapture.setImageResource(android.R.drawable.ic_media_play) // Đổi icon
            return
        }
        ...
        // Cần quyền RECORD_AUDIO nếu muốn quay cả tiếng
        if (PermissionChecker.checkSelfPermission(this, android.Manifest.permission.RECORD_AUDIO) == PermissionChecker.PERMISSION_GRANTED)
        {
            recording = videoCapture.output
                ...
                .withAudioEnabled() // Bật âm thanh
                .start(cameraExecutor) { recordEvent ->
                    when (recordEvent) {
                        is VideoRecordEvent.Start -> {
                            runOnUiThread { binding.btnCapture.setImageResource(android.R.drawable.ic_media_pause) } // Đổi icon
                        }
